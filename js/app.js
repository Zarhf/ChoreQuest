const app = {
        ...
        if (!app.mainUser) {
            ...
            if (app.data.meta.owner === auth.user.email) {
                if (!sessionStorage.getItem('system_version_pushed')) {
                    db.setSystemConfig(79); // Push v79
    ...
    showView(viewId) {
        ...
        const view = document.getElementById(viewId); if (view) view.classList.remove('hidden');
    },
    ...
    askConfirm(message, callback) {
        ...
        btn.onclick = () => { if (app._pendingAction) app._pendingAction(); app.hideModals(); app._pendingAction = null; };
    },
    ...
    async unclaimQuest(instanceId) {
        const quest = app.data.activeQuests.find(q => q.id === instanceId);
        if (!quest) return;
        quest.assignedTo = null; // Back to 'For all'
        await app.save();
    },
    ...
    render() {
        ...
        if (squires.length > 0) {
            switchBtn.classList.remove('hidden'); const rotationList = [app.mainUser, ...squires];
            const currentIndex = rotationList.findIndex(u => u.id === app.currentUser.id);
            const nextUser = rotationList[(currentIndex + 1) % rotationList.length];
    ...
    renderBoard() {
        ...
        const html = (q, up) => {
            ...
            let actionButtons = '';
            if (!up) {
                // 1. Validation Button: Only for the assignee or if nobody is assigned
                if (isMe || isNobody) {
                    actionButtons += `<button class="quest-action-btn btn-complete" onclick="event.stopPropagation(); app.askConfirm('Terminer cette quête ?', () => app.completeTask('${q.id}'))" title="Valider">✅</button>`;
                }
                
                // 2. Claim / Abandon / Steal Button
                if (isMe) {
                    actionButtons += `<button class="quest-action-btn btn-abandon" onclick="event.stopPropagation(); app.askConfirm('Abandonner cette quête ?', () => app.unclaimQuest('${q.id}'))" title="Abandonner">❌</button>`;
                } else if (isNobody) {
                    actionButtons += `<button class="quest-action-btn btn-claim" onclick="event.stopPropagation(); app.askConfirm('Prendre cette quête ?', () => app.claimQuest('${q.id}'))" title="☝️ Je prends">☝️</button>`;
                } else if (isStealable) {
                    actionButtons += `<button class="quest-action-btn btn-steal" onclick="event.stopPropagation(); app.askConfirm('🥷 VOLER cette quête ?', () => app.claimQuest('${q.id}'))" title="🥷 Voler">🥷</button>`;
                }
            }
